...
const MenuScreen: React.FC<MenuScreenProps> = ({
  ...
  return (
    <div className="h-screen bg-surface-primary text-white font-sans flex flex-col overflow-hidden selection:bg-blue-500/30 relative">
        ...
        {showHistoryModal && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                ...
                <motion.div 
                    ...
                    className="relative w-full max-w-2xl bg-surface-primary border border-border-default rounded-3xl shadow-2xl overflow-hidden max-h-[80vh] flex flex-col"
                >
                    <div className="p-6 border-b border-border-default flex justify-between items-center bg-surface-secondary">
                        <h2 className="text-2xl font-bold flex items-center gap-2"><CheckIcon /> Play History</h2>
                        <button onClick={() => setShowHistoryModal(false)} className="p-2 hover:bg-white/10 dark:hover:bg-surface-tertiary rounded-full">✕</button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-6 space-y-4">
                        {playedSongsList.length === 0 ? (
                            <div className="text-center text-text-secondary py-10"><p>No songs played yet.</p></div>
                        ) : (
                            playedSongsList.map((item, idx) => (
                                <div key={item.id} className="flex items-center justify-between p-4 bg-surface-secondary/50 rounded-xl border border-border-default hover:border-blue-500/30 transition-all">
                                    <div className="flex items-center gap-4">
                                        <div className="w-10 h-10 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center font-bold">
                                            {item.stats.stars > 0 ? '★' : idx + 1}
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-text-primary text-lg">{item.song?.title || "Unknown"}</h3>
                                            <p className="text-xs text-text-secondary">{item.song?.artist}</p>
                                        </div>
                                    </div>
      ...
      <AnimatePresence>
        <motion.header 
            ...
            className="sticky top-0 z-50 w-full border-b border-border-default bg-surface-primary/80 backdrop-blur-xl"
        >
            <div className="max-w-6xl mx-auto px-6 h-16 flex justify-between items-center">
                ...
                <div className="hidden md:flex items-center bg-surface-secondary rounded-full p-1 border border-border-default">
                    {['home', 'learning', 'library'].map(tab => (
                        <button 
                            key={tab}
                            onClick={() => setActiveTab(tab)}
                            className={`px-6 py-2 rounded-full text-sm font-medium transition-all ${activeTab === tab ? 'bg-white dark:bg-surface-primary text-black dark:text-text-primary shadow-sm' : 'text-text-secondary hover:text-text-primary'}`}
                        >
                ...
                <div className="flex items-center gap-4">
                    <div className="relative">
                        <button onClick={() => setShowUserMenu(prev => !prev)} className="w-9 h-9 rounded-full bg-gradient-to-br from-gray-700 to-gray-900 border border-white/20 dark:border-border-default flex items-center justify-center text-sm font-bold shadow-inner">
                            {currentUser.name.charAt(0).toUpperCase()}
                        </button>
                        {showUserMenu && (
                            <>
                            <div className="fixed inset-0 z-40" onClick={() => setShowUserMenu(false)} />
                            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="absolute top-12 right-0 w-64 bg-surface-secondary rounded-2xl shadow-2xl border border-border-default z-50 p-2">
                                <div className="p-4 border-b border-white/5 dark:border-border-default mb-2">
                                    <p className="font-bold text-text-primary">{currentUser.name}</p>
                                    <div className="mt-2 flex gap-2 text-xs"><span className="px-2 py-1 bg-white/5 dark:bg-surface-tertiary rounded">Lvl {currentUser.level}</span></div>
                                </div>
                                {currentUser.isAdmin && <button onClick={() => setAppState(AppState.SETTINGS)} className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-white/5 dark:hover:bg-surface-tertiary text-sm"><CogIcon /> Settings</button>}
            ...
            <AnimatePresence mode="wait">
                ...
                {activeTab === 'home' && (
                    <motion.div key="home" variants={containerVariants} initial="hidden" animate="visible" exit="hidden" className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <motion.div variants={itemVariants} className="p-6 rounded-3xl bg-surface-secondary border border-border-default shadow-sm">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="p-2 bg-yellow-500/20 rounded-lg text-yellow-500"><LightningIcon/></div>
                                    <h3 className="font-bold text-text-secondary text-sm uppercase">Streak</h3>
                                </div>
                                <p className="text-3xl font-bold text-text-primary">{currentUser.streak} days</p>
                            </motion.div>
                            <motion.div variants={itemVariants} onClick={() => setShowHistoryModal(true)} className="cursor-pointer p-6 rounded-3xl bg-surface-secondary border border-border-default shadow-sm hover:border-blue-500 transition-colors">
                                <div className="flex items-center gap-4 mb-2">
                                    <div className="p-2 bg-green-500/20 rounded-lg text-green-500"><CheckIcon/></div>
                                    <h3 className="font-bold text-text-secondary text-sm uppercase">Mastered</h3>
                                </div>
                                <p className="text-3xl font-bold text-text-primary">{Object.values(currentUser.progress).filter((p: SongStats) => p.stars === 3).length}</p>
                            </motion.div>
                        </div>
                    </motion.div>
                )}
                ...
                {activeTab === 'library' && (
                    <motion.div key="library" variants={containerVariants} initial="hidden" animate="visible" exit="hidden" className="space-y-8 pb-20">
                        <div className="bg-surface-secondary p-1.5 rounded-2xl border border-border-default flex items-center shadow-lg">
                            <div className="pl-4 text-text-secondary"><SearchIcon /></div>
                            <input 
                                ...
                                className="w-full bg-transparent border-none focus:ring-0 text-lg px-4 py-2 placeholder-text-secondary text-text-primary"
                            />
                        ...
                        {/* AI Lesson Generator */}
                        <div className="bg-surface-secondary rounded-3xl p-8 border border-border-default shadow-xl relative overflow-hidden">
                            ...
                            <div className="relative z-10 flex flex-col md:flex-row gap-8">
                                <div className="md:w-1/3 space-y-4">
                                    ...
                                    <h3 className="text-2xl font-bold text-text-primary">Custom AI Lesson</h3>
                                    <p className="text-text-secondary leading-relaxed">Tell Gemini what you want to practice.</p>
                                ...
                                <div className="md:w-2/3 flex flex-col gap-6">
                                    <div>
                                        <label className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-3 block">Difficulty</label>
                                        <div className="grid grid-cols-3 gap-3">{levels.map(lvl => <button key={lvl} onClick={() => setLessonLevel(lvl)} className={`py-3 px-4 rounded-xl text-sm font-bold border ${lessonLevel === lvl ? 'bg-purple-600 border-purple-600 text-white' : 'bg-surface-tertiary text-text-secondary border-transparent'}`}>{lvl}</button>)}</div>
                                    </div>
                                    <div>
                                        <label className="text-xs font-bold text-text-secondary uppercase tracking-wider mb-3 block">Style</label>
                                        <div className="grid grid-cols-4 gap-3">{genres.map(g => <button key={g} onClick={() => setLessonGenre(g)} className={`py-3 px-4 rounded-xl text-sm font-bold border ${lessonGenre === g ? 'bg-blue-600 border-blue-600 text-white' : 'bg-surface-tertiary text-text-secondary border-transparent'}`}>{g}</button>)}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
